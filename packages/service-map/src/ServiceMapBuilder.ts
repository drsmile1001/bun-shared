...
> {
  ...
  private readonly registeredMap: Record<string, unknown> = {};

  private constructor() {}

  get registered(): TRegistered {
    return this.registeredMap as TRegistered;
  }
  ...
  register(key: string, value: unknown) {
    ...
    if (typeof value === "function") {
      try {
        this.registeredMap[key] = value(this.registeredMap as TRegistered);
      } catch (err) {
  ...
  > {
    ...
    for (const [key, service] of Object.entries(this.registeredMap)) {
